...
class RandomHotbarPerSlotAgentStart(Handler):
    """九个快捷栏位各自独立：按概率是否放入物品；放入则从池中随机类型与数量。"""

    def __init__(
        self,
        item_pool: List[Dict[str, Union[str, int]]],
        slot_occupancy_prob_range=(0.35, 0.85),
        quantity_range=(1, 64),
    ):
        self.item_pool = item_pool
        self.slot_occupancy_prob_range = slot_occupancy_prob_range
        self.quantity_range = quantity_range

    def to_string(self) -> str:
        return "random_hotbar_per_slot_agent_start"

    def xml_template(self) -> str:
        p = random.uniform(
            self.slot_occupancy_prob_range[0], self.slot_occupancy_prob_range[1]
        )
        lines = ["<Inventory>"]
        for slot in range(9):
            if random.random() < p:
                ...
                q = random.randint(self.quantity_range[0], self.quantity_range[1])
                lines.append(
                    f'<InventoryObject slot="{slot}" type="{t}" quantity="{q}"/>'
                )
        lines.append("</Inventory>")
        return "\n".join(lines)
